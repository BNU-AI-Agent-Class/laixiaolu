...
def search(keyword, path="."):                                      # 4b. 工具：在项目里按关键词搜索文件内容
    """遍历 path 下所有文件，返回包含 keyword 的行（文件名:行号:内容）"""
    hits, max_hits = [], 30                                          # 最多返回 30 条，避免上下文爆炸
                    ...
                    for lineno, line in enumerate(f, 1):
                        if keyword in line:
                            ...
                            if len(hits) >= max_hits:
                                return "\n".join(hits)
            except (UnicodeDecodeError, OSError, PermissionError):     # 二进制/读不了就跳过
                continue
    return "\n".join(hits) if hits else f"未找到包含「{keyword}」的内容"
...
- 搜索项目内容：{"tool": "search", "args": {"keyword": "...", "path": "..."}}
- 完成时：{"done": "总结"}
优先用 read_file / write_file 处理文件，用 search 查找代码位置，它们比 bash 更安全可控。"""  # 8. 系统提示：告诉模型有哪些工具、何时用

if __name__ == "__main__":                                             # 9. 只在直接运行时启动交互循环，方便 import 测试
